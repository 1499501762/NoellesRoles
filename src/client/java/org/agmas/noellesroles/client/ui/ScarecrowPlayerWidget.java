...
import org.agmas.noellesroles.packet.ScarecrowC2SPacket;
import org.jetbrains.annotations.NotNull;

import java.awt.*;
import java.util.UUID;

public class ScarecrowPlayerWidget extends ButtonWidget{
    public final LimitedInventoryScreen screen;
    public final AbstractClientPlayerEntity disguiseTarget;

    public ScarecrowPlayerWidget(LimitedInventoryScreen screen, int x, int y, @NotNull AbstractClientPlayerEntity disguiseTarget, int index) {
        super(x, y, 16, 16, disguiseTarget.getName(), (a) -> {
            if ((AbilityPlayerComponent.KEY.get(MinecraftClient.getInstance().player)).cooldown == 0) {
                ...
                ClientPlayNetworking.send(new ScarecrowC2SPacket(disguiseTarget.getUuid()));
            }
